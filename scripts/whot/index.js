...
setInterval(function() {
	...
	function loadParticipants(x) {
	
		if(x < participantsArray.length) {
	
			//get participants usercode from server via AJAX
	          
			$("#participantsUsercode").load("../session/" + sessionStorage.getItem("gameStatus") + "/usercodes/" + participantsArray[x].toLowerCase().replace(/ /g,  "") + ".txt", function() {
		
				participantsOnArray += '<div class="eachParticipant"> <img src="../images/profilePictures/' + $("#participantsUsercode").html() + '.jpg" class="eachParticipantPicture"></img>  <div class="eachParticipantName">' + participantsArray[x] + '</div> </div>';
			
				loadParticipantsCheck(x);
	...
	function loadParticipantsCheck(x) {
...
// display player as well as successive player on heading
	...
	setInterval(function() {
		...
		// determine value of nextPlayerIndex variable ( currently unuseful)
		...
		// determine value of previousPlayerIndex variable 
	
		if(Number($("#currentPlayer").html()) !== 0) {
		
			previousPlayerIndex = Number($("#currentPlayer").html()) - 1;
		
		}
		
		else {
		
		previousPlayerIndex = participantsArray.length - 1;
			...
			// change appearance of commanded player 
	 				...
	 				if($("#currentCommand").html() == "1") {
						...
						$("#threePicker").load("../session/" + sessionStorage.getItem("gameStatus") + "/threePicker.txt", function() {
							...
							while(i < $(".eachParticipant").length)  {
							
								if($(".eachParticipant .eachParticipantName").eq(i).html().trim().toLowerCase().replace(/ /g,  "") == $("#threePicker").html()) {
							...
							if(localStorage.getItem("accountInformation") == $("#threePicker").html()) {
						...
						if(participantsArray.indexOf(localStorage.getItem("scrabblitUsername")) == currentPlayerIndex) {
							
							// load whot requester index file 
					
							$("#whotRequesterIndex").load("../session/" + sessionStorage.getItem("gameStatus") + "/whotRequesterIndex.txt", function() {
					
								// load whot shape file 
					
								$("#whotShapeChecker").load("../session/" + sessionStorage.getItem("gameStatus") + "/whotShape.txt", function() {
					
									remark(participantsArray[Number($("#whotRequesterIndex").html())] + " needs " + $("#whotShapeChecker").html());
	...
	// code which will clear remark at intervals in case of pick two or other remarks which get stuck on screen
	...
	$("#refresh").click(function() {
		...
		$(this).html('<div id="refreshingIcon"></div>');
