function activateCardEventListener() {
		...
		if(Number($("#currentPlayer").html()) + 1 < participantsArray.length) {
			...
			$("#firstPlayChecker").load("../session/" + sessionStorage.getItem("gameStatus") + "/playerStatus/" + localStorage.getItem("accountInformation") + ".txt", function() {
				...
				if($("#firstPlayChecker").html() !== "true") {
					...
					// allow play of whot( I need) regardless of whether he was issued a whot command or not
					
					if(clickedCardShape == "Whot") {
					...
					else if($("#whotCommandChecker").html() == localStorage.getItem("accountInformation")) {
					...
					$("#whotShapeChecker").load("../session/" + sessionStorage.getItem("gameStatus") + "/whotShape.txt", function() {
						...
						// check if shape of clickedcard matches that of what shape
							...
							// if it does play as current player and empty whotCommand file and whotShape file
	...
	$.ajax({
		...
		data: {
			...
			request: "setWhotCommand",
			name: ""
		},
		cache: false
	});
	
			// empty whot shape through AJAX
	...
	$.ajax({
		...
		setTimeout(function() {
			...
			playAsCurrentPlayer(clickedCardNumber);
								...
								toast( "Whot command requires you to play " + $("#whotShapeChecker").html());
	...
	$.ajax({
		...
		cache: false
				...
				// if status is true ( he has dropped at least one card ) 
		...
		setTimeout(function() {
			...
			$("#firstPlayChecker").load("../session/" + sessionStorage.getItem("gameStatus") + "/playerStatus/" + localStorage.getItem("accountInformation") + ".txt", function() {
				...
				if($("#firstPlayChecker").html() !== "true") {
					...
					// allow play of whot( I need) regardless of whether the shape or number is accurate 
					...
					else if( $("#cardStack").children(".cards").eq($("#cardStack .cards").length - 1).children(".largeShape").children().attr("class") == clickedCardShape || $("#cardStack").children(".cards").eq($("#cardStack .cards").length - 1).children(".number").html() == clickedCardNumber ) {
	...
	// update playerStatus of himself as true
		...
		$.ajax({
			...
			$("#whotCommandChecker").load("../session/" + sessionStorage.getItem("gameStatus") + "/whotCommand.txt", function() {
				...
				// check if he has been issued a whot command
					
				if($("#whotCommandChecker").html() == localStorage.getItem("accountInformation")) {
					
					// if so increment whotCommand file just because the card picker has been issued a whot command and was not able to settle it and then go for picking a card then the next player will be bounded by the whot command 
